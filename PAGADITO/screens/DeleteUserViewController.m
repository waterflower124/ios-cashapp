...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGRect frame = cell.deletetableviewcell.frame;
    frame.origin.x = 0;
    cell.deletetableviewcell.frame = frame;
...
- (IBAction)deleteUserOKButtonAction:(id)sender {
    ...
    [sessionManager POST: globals.server_url parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
        ...
        if(status) {
            if(globals.selected_language == 0) {
                [self displayAlertView:@"¡Éxito!" :[NSString stringWithFormat:@"Eliminaste al usuario %@ con éxito.", self.deleting_user[4]]];
            } else {
                [self displayAlertView:@"Success!" :[NSString stringWithFormat:@"You succesfully deleted user %@.", self.deleting_user[4]]];
            }
        } else {
            if(globals.selected_language == 0) {
